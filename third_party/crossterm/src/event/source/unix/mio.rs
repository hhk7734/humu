    ...
    fn try_read(&mut self, timeout: Option<Duration>) -> io::Result<Option<InternalEvent>> {
        ...
        loop {
            ...
            if self.events.is_empty() {
                ...
                return Ok(self.parser.flush_pending_esc());
            }
...
impl Parser {
    fn advance(&mut self, buffer: &[u8], more: bool) {
        for (idx, byte) in buffer.iter().enumerate() {
            ...
            match parse_event(&self.buffer, more) {
                Ok(Some(ie)) => {
                    if self.buffer == [b'\x1B'] && is_plain_esc_key(&ie) && !more {
                        // Keep a trailing ESC buffered until the next poll cycle so
                        // split CSI-u sequences like ESC + "[12618;5u" are not
                        // misclassified as a standalone Esc key followed by text.
                    } else {
    ...
    fn flush_pending_esc(&mut self) -> Option<InternalEvent> {
...
fn is_plain_esc_key(event: &InternalEvent) -> bool {
    matches!(
        event,
        InternalEvent::Event(Event::Key(key_event))
...
mod tests {
    ...
    #[test]
    fn parser_keeps_split_csi_u_escape_prefix_buffered() {
        let mut parser = Parser::default();
        parser.advance(b"\x1B", false);
        assert!(parser.next().is_none());

        parser.advance(b"[12618;5u", false);
        assert_eq!(
            parser.next(),
    ...
    #[test]
    fn parser_flushes_plain_escape_when_no_followup_arrives() {
        let mut parser = Parser::default();
        parser.advance(b"\x1B", false);
        assert_eq!(
            parser.flush_pending_esc(),
            Some(InternalEvent::Event(Event::Key(KeyCode::Esc.into())))
        );
    }
}
